...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    iteration = 0
    while Path("models/torch/run_{}_epoch_{}.pk".format(iteration, epoch)).is_file(): iteration+=1
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            # store at every iter or at log iterval iters??
            losses.append(loss)
            parameters.append(model.state_dict())

    if args.save_full:
        outfile = "models/torch/run_{}_epoch_{}.pk".format(iteration, epoch)
...
def main():
    ...
    parser.add_argument('--save-full', action='store_true', default=False,
                        help='Save incremental weights and losses')
